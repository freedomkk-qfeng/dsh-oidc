...
| --- | --- | --- | --- | --- |
| `0.1.0` | 22、24 | `0.1.2-rc.1` / `a66e4702…` | `^0.84.2`（锁定为 `0.84.4`） | 精确 peer 闭包、Host/Client 契约检查、干净 npm 安装和隔离 Web/RPC 验收。 |
...
当前受支持线是 `0.1.0` 与 `package.json` 中声明的精确 DSH `0.1.2-rc.1` peer 组合；alpha 行只保留历史验证记录。其他版本即使通过显式 peer override 安装，也属于未经测试的组合。
