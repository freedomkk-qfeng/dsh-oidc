...
import { modelCapabilitySummary, runtimeModelDraft, serializeRuntimeModelDraft } from './presentation.js'
...
const copy: Record<string, string> = isChinese ? {
  ...
  displayName: '显示名称', baseModelID: '基础模型 ID', contextWindow: '上下文窗口', maxOutput: '最大输出', modalities: '输入模态', reasoningLevels: '思考强度',
  ...
  profileNotice: 'Web 版本从受信 Enterprise Profile 读取机构与模型目录；如需修改，请更新 Profile 后重启 DSH。',
} : {
  ...
  displayName: 'Display name', baseModelID: 'Base model ID', contextWindow: 'Context window', maxOutput: 'Max output', modalities: 'Input modalities', reasoningLevels: 'Reasoning effort',
  ...
  profileNotice: 'The Web edition reads organizations and models from a trusted Enterprise Profile. Update the Profile and restart DSH to change them.',
}
...
const capabilityLabel = (value: string) => ({ xhigh: isChinese ? '极高' : 'Extra high', max: 'Max', high: isChinese ? '高' : 'High', medium: isChinese ? '中' : 'Medium', low: isChinese ? '低' : 'Low' } as Record<string, string>)[value] || value
...
export function ManagedProviderCard({ service, configuration }: any) {
  ...
  const run = async (operation: () => Promise<any>, message = '') => {
    ...
    try {
      ...
      } else if (result?.profileID) setStatuses(current => ({ ...current, [result.profileID]: result }))
      if (message) setNotice(message)
      return result
    } catch (cause: any) { setError(cause?.message || String(cause)); return null }
    finally { setBusy(false) }
  }
  const signIn = (profileID: string) => run(async () => {
    const result = await service.begin(profileID)
    if (result.mode === 'redirect') { window.location.assign(result.authorizationURL); return null }
  ...
  const canManageProfiles = management.capabilities.manageProfiles === true
  const canManageModels = management.capabilities.manageModels === true
  ...
  const providerCards = configured.length > 0 ? h('div', { style: { display: 'grid', gap: 10 } }, ...configured.map((profile: any) => {
    ...
    const connected = profile.enabled && status?.credentialReady === true
    const modelBadges = profile.runtime.models.length > 0
      ...
      : [h('span', { key: 'empty', style: { color: textTertiary, fontSize: 11 } }, copy.noModels)]
      ...
      h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 } },
        h('div', { style: { minWidth: 0 } },
          h('div', { style: { display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' } },
            ...
            h('span', { style: { borderRadius: 999, padding: '2px 7px', background: profile.builtIn ? '#f5e9e8' : '#eef1f6', color: profile.builtIn ? accent : '#536174', fontSize: 11 } }, profile.builtIn ? copy.verified : copy.custom),
        ...
        h('div', { style: { display: 'flex', gap: 7, flex: 'none', flexWrap: 'wrap', justifyContent: 'flex-end' } },
          h('button', { type: 'button', disabled: busy, style: buttonStyle, onClick: () => openDetail(profile) }, copy.configure),
    ...
    h('p', { style: { margin: '6px 0 0', color: textSecondary, fontSize: 12, lineHeight: 1.55 } }, copy.emptyHint))
  ...
  h('div', { style: { marginTop: 8, fontSize: 11 } },
    ...
    ...(draft.reasoningSupported ? ['xhigh', 'max', 'high', 'medium', 'low'].map(value => h('label', { key: value, style: { marginRight: 11 } },
  ...
  const detailDialog = detail && h('div', { style: panelStyle, onMouseDown: (event: any) => { if (event.target === event.currentTarget && !busy) setDetailID('') } },
    h('div', { role: 'dialog', 'aria-modal': 'true', 'aria-label': `${detail.displayName} ${copy.configure}`, style: cardStyle },
      ...
      }) : [h('div', { key: 'empty', style: { border: `1px dashed ${border}`, borderRadius: 11, padding: 14, color: textTertiary, fontSize: 12 } }, copy.noModels)])),
